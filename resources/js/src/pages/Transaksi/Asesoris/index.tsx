...
import { PlusIcon, XIcon, ArrowUpIcon, ArrowDownIcon } from 'lucide-react';
import sortBy from 'lodash/sortBy';

interface Asesoris {
    id_asesoris: number; id_model: number; stok: number; digunakan: number; rusak: number;
    model?: any;
}

const AsesorisPage = () => {
    const dispatch = useDispatch();
    const [items, setItems] = useState<Asesoris[]>([]);
    ...
    const [initialRecords, setInitialRecords] = useState<Asesoris[]>([]);
    const [records, setRecords] = useState<Asesoris[]>([]);
    const [sortStatus, setSortStatus] = useState<DataTableSortStatus>({ columnAccessor: 'model.nama_model', direction: 'asc' });
    const [search, setSearch] = useState('');

    useEffect(() => { dispatch(setPageTitle('Asesoris')); fetchData(); }, []);
    ...
    const fetchData = async () => {
        try {
            const [asesorisRes, penggunaRes] = await Promise.all([
                axios.get('/api/asesoris'),
                axios.get('/api/penggunas'),
            ]);
            setItems(asesorisRes.data);
            setInitialRecords(asesorisRes.data);
            setPenggunas(penggunaRes.data);
        } catch (error) { console.error(error); }
    };

    const openMasuk = (item: Asesoris) => {
        setModalType('masuk');
        setFormData({ id_asesoris: item.id_asesoris, jumlah: 1 });
        setModal(true);
    };

    const openKeluar = (item: Asesoris) => {
        setModalType('keluar');
        setFormData({ id_asesoris: item.id_asesoris, jumlah: 1, id_pengguna: '' });
        setModal(true);
    };
    ...
    const handleSave = async () => {
        try {
            if (modalType === 'masuk') {
                await axios.post('/api/asesoris/stok-masuk', formData);
            } else {
                if (!formData.id_pengguna) return;
                await axios.post('/api/asesoris/checkout', formData);
            }
    ...
    return (
        <div className="panel">
            <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                <h5 className="font-semibold text-lg dark:text-white-light">Asesoris</h5>
